...
def calculate_fid_given_paths(train_path, test_path, stat_path = 'checkpoints', batch_size = 10, dims = 2048, for_train=False):
...
def get_offline_fid(train_path, test_path, stat_path='checkpoints'):
    batch_size = 10
    dims = 2048
...
if __name__=='__main__':
    ...
    train_path = "/opt/data/private/zhasion_gan/dataset/train_resized/imgs"
    for submit_name, test_path in path_list.items():
        if "opt" not in test_path:
            test_path = os.path.join("/opt/data/private/zhasion_gan", test_path)
        fid_score = calculate_fid_given_paths(train_path, test_path)
        print(submit_name, fid_score)