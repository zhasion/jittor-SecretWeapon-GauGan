...
class TrainOptions(BaseOptions):
    def initialize(self, parser):
        ...
        # for training # TODO:
        parser.add_argument('--pg_niter', type=int, default=180, 
                            help='# of iter uses mid supervision D') 
        parser.add_argument('--pg_strategy', type=int, default=1, 
                            help=' 0 is ont using pg, 1 is classic pg, 2 is all then discard strategy')
        parser.add_argument('--pg_lr_decay', type=int, default=1, 
                            help='learning rate decay at every resolution transition.') 
        parser.add_argument('--diff_aug', type=str, default='', 
                            help='Diff augment policy: color,crop,translation')
        
        parser.add_argument('--one_pg_D', action='store_true', 
                            help='Only use one last D in pg training')
        parser.add_argument('--reverse_map_D_pg', action='store_true', 
                            help='Only use one last D in pg training')
        parser.add_argument('--reverse_map_D_ft', action='store_true', 
                            help='Only use one last D in fine turning')
        parser.add_argument('--no_inception_loss', action='store_true',
                            help='if specified, do *not* use Inception feature matching loss')
        parser.add_argument('--lambda_inception', type=float, default=10.0, 
                            help='weight for inception loss')
