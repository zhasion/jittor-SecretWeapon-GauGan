...
class SPADEResnetBlock(nn.Module):
    ...
    def execute(self, x, seg, style_latent):
        x_s = self.shortcut(x, seg, style_latent)
        
        dx = self.conv_0(self.actvn(self.norm_0(x, seg, style_latent)))
        dx = self.conv_1(self.actvn(self.norm_1(dx, seg, style_latent)))

        out = x_s + dx
        return out

    def shortcut(self, x, seg, style_latent):
        if self.learned_shortcut:
            x_s = self.conv_s(self.norm_s(x, seg, style_latent))
        else:
            x_s = x
        return x_s
